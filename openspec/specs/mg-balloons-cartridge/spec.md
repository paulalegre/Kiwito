# mg-balloons-cartridge Specification

## Purpose

Defines the first real "cartridge" built on top of the frozen `minigame-contracts` capability
(`shared/contracts/MinigameBase`/`MinigameResult`/`LevelConfig` — consumed unmodified, not touched by this
capability). Its job is to prove, end to end, the platform promise from `docs/GDD_MVP.md` §4 — "add a
minigame, or a new educational dimension inside one, without touching architecture code" — by shipping
`MATCH_ANY` and `MATCH_COLOR` as pure `.tres` data on top of one unmodified cartridge script, and by
embodying the Zero-Punishment product invariant (`CLAUDE.md`; `docs/GDD_MVP.md` §3) at the gameplay level for
the first time: an incorrect tap gets neutral feedback only and never ends, restarts, or penalizes the
level.
