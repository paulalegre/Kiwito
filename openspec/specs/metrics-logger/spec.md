# metrics-logger Specification

## Purpose

`MetricsLogger` (autoload) is the domain owner for local gameplay telemetry: buffering finished minigame
session results in memory, flushing them to a local telemetry file only at safe points (session end, app
backgrounding/closing), and doing so atomically and independently of `SaveManager`. It performs local file
I/O only — no network transmission — and a flush failure must never crash or block gameplay.
