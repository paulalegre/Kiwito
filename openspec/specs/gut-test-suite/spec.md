# gut-test-suite Specification

## Purpose

Gives pure-logic systems — those decoupled from the `SceneTree` — automated coverage that runs without the
full game, using GUT (Godot Unit Test) as named in `docs/Core_Architecture.md` §7. That section also fixes
the order coverage must be added in: save robustness first, then progression milestones, then `LevelConfig`
parsing. `docs/Propuesta_MVP.md` §8 gives the reason for that ordering — "se testea primero lo que corrompe
datos del niño" — a corrupted or lost save is the one failure mode that harms a child's experience across
every session, so it is tested before anything else. This capability exists so that both the coverage and
the priority order itself are checkable contracts: a later change skipping ahead to progression or
`LevelConfig` tests before save robustness is complete is a violation, not just a missed convention.
